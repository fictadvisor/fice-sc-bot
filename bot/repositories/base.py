...
class BaseRepository(Generic[K, V]):
    ...
    async def delete_by_filter(self, model_filter: V):
    ...
    def _set_filter(
        ...
        if model_filter is not None:
            query = query.filter_by(**model_filter.model_dump(exclude_unset=True))
    ...
    async def check_exists(self, model_filter: V) -> bool:
